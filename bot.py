...
class SOSModal(discord.ui.Modal, title="🆘 SOS Emergency"):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        await create_ticket(interaction, "🆘 SOS Emergency", RED, fields_dict)
...
# ══════════════════════════════════════════════════════
#   CLOSE TICKET CONTROL PANEL (UNBREAKABLE STAFF-ONLY)
# ══════════════════════════════════════════════════════
class CloseTicketView(discord.ui.View):
    ...
    async def close_ticket(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        # Absolute Role Validation Check (Database bypass fix)
        is_staff = staff_role in member.roles if staff_role else False
        is_admin = member.guild_permissions.administrator

        # CRITICAL HARD LOCK: If they are NOT staff and NOT server admin, block instantly
        if not is_staff and not is_admin:
            await interaction.response.send_message(f"❌ Only {STAFF_ROLE_NAME} staff can close this ticket!", ephemeral=True)
            return

        # Request verified, proceed with channel shutdown
        await interaction.response.defer()
        
        # Find owner dynamically from DB via Channel ID to update logs/status
        db = load_db()
        owner_id = None
        ...
        close_embed = discord.Embed(
            ...
            description=f"Closed by Staff: {member.mention}\nDeleting in **5 seconds.**",
        ...
        try:
            await interaction.channel.delete(reason=f"Ticket closed by staff member {member}")
...
async def setup_tickets(ctx):
    ...
    embed = discord.Embed(
        ...
        description=(
            ...
            "━━━━━━━━━━━━━━━━━━━━━━\n"
            "**1 ticket per person.** Tickets can only be closed by Authorized Staff."
        ),
